...
from .models import Game, StartGame, MoveInput, get_model_safe
from .crud import start_new_game, list_games_from_db, get_game_by_id, delete_games_from_db, join_new_game, update_game
...
async def websocket_game_endpoint(websocket: WebSocket, game_id: PyObjectId) -> None:
    ...
    try:
        while True:
            ...
            # make move 
            make_move(game, move.col)

            # update DB
            updated_game = await update_game(game.id, game.model_dump())
