...
class MongoDBClient:
    ...
    async def delete_many(self, model: MongoDBModel) -> DeleteResult:
        collections = self.get_collection(model)
        return await collections.delete_many({}) 
    
    async def update_one(self, model: MongoDBModel, id: PyObjectId, data: dict[str, Any]) -> UpdateResult: 
        collection = self.get_collection(model)
