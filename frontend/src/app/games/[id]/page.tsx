...
export default function PlayGame({ params }: { params: { id: string } }) {
    ...
    return (
        <div
            ...
            py-4
            px-8 md:px-10 lg:px-16 lx:px-18 3xl:px-12 4xl-px-32
    ...
    return (
        ...
        >
            ...
            <p className="text-lg 3xl:text-xl font-bold ">
                ...
                <span className="text-yellow-400 dark:text-blue-500 drop-shadow-2xl">{gameData.player2}
                    {gameData.player2}
                </span>
            </p>
            <p>{gameStatus}</p>
...
function GameBoard({gameData, playerName, ws}: {gameData: GameData; playerName: string; ws: WebSocket | null}) {
    ...
    const handleColunmHover = (colIndex: number) => {
        setHighLightedColumn(colIndex);
    };

    const handleColunmLeave = () => {
                        ...
                        <tr key={`row-${rowIndex}`}>
                            {row.map((cell: number, colIndex: number) => (
                                <GameBoardCell
                                    ...
                                    handleColunmHover={handleColunmHover}
                                    handleColunmLeave={handleColunmLeave}
                                />
                            ))}
                        </tr>
                    ))}
...
function GameBoardCell({
    ...
    handleColunmHover: (colIndex: number) => void;
    handleColunmLeave: () => void;
}) {
    // highlight cell logic
    let toHighLight = false;
    if (gameData.board[rowIndex][colIndex] == 0 && !gameData.finished_at &&
        gameData.next_player_to_move_username == playerName && highLightedColumn === colIndex
    ) {
        toHighLight = true;
    }
    ...
    return (
        <td 
            ...
            onMouseEnter={() => handleColunmHover(colIndex)}
            onMouseLeave={handleColunmLeave}
        >
            <button
                className={`
